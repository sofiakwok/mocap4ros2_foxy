  ...
  eprosima::fastcdr::Cdr & cdr)
{
  ...
  // Field name: rigidbodies
  {
    const message_type_support_callbacks_t * callbacks =
      static_cast<const message_type_support_callbacks_t *>(
      ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
        rosidl_typesupport_fastrtps_c, mocap4r2_msgs, msg, RigidBody
      )()->data);
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: rigidbodies
  {
    ...
    if (!mocap4r2_msgs__msg__RigidBody__Sequence__init(&ros_message->rigidbodies, size)) {
      return "failed to create array for field 'rigidbodies'";
    }
    auto array_ptr = ros_message->rigidbodies.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
static size_t _RigidBodies__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_mocap4r2_msgs__msg__RigidBodies(
    full_bounded, 0);
}
