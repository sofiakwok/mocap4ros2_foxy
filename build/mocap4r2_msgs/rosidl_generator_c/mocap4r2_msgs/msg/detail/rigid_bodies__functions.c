  ...
  mocap4r2_msgs__msg__RigidBodies__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mocap4r2_msgs__msg__RigidBodies * data =
      (mocap4r2_msgs__msg__RigidBodies *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mocap4r2_msgs__msg__RigidBodies__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mocap4r2_msgs__msg__RigidBodies__fini(&data[i]);
        }
        free(data);
        return false;
      }
